...
export function ListView(model, events) {
  ...
  function itemIsVisible(completed) {
    if (model.filterBy === 'all') {
      return true;
    }
    if (model.filterBy === 'completed') {
      return completed;
    }

    if (model.filterBy === 'active') {
      return !completed;
    }
  }
  ...
  function render() {
    ...
    model
      ...
      .forEach((todo, index) => {
        ...
        if (itemIsVisible(todo.completed)) {   
  ...
  events.onPropertyChange('filterBy', render);
